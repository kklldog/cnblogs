...
```
blazor的注入同样分Transient、Scope、Singleton等生命周期。这里我们注册HttpClient为Transient，并且配置baseAddress为https://localhost:6001，这是Api site的地址。
## 实现学生列表（student/list）
...
```
这个文件大体上看跟razor page的页面差不多，html主体使用razor语法渲染。但是还是有很大的不同，让我们从头开始一个个的解释:
```
@page "/student/list"
```
...
@using BlazorWebAssemblyApp.Model
```
@using指令不多说了，引用namespace，这个跟razor page是一样的
```
@inject HttpClient Http
```
