...
```
使用 docker 命令运行 consul 的 server 模式。这里有两行 docker 命令。其中第一行启动启动一个默认的第一个节点。   
* -server 启动为Server模式
...
```
我们有了 Server 集群，现在可以开始建立 Consul 的 client节点，然后加入集群。启动 Consul client 的命令跟启动 Consul server 的差不多。去掉了 -server 就代表这个 agent 为 client 模式。
## 使用 docker-compose 运行 Consul
上面分步骤演示了如何使用 docker 命令来运行 Consul 集群。一行行敲还是太麻烦，为了简化部署，我们整理成了 docker-compose 启动文件。
```
version: '3.9'
services:
...
如果什么的操作一切正常，在浏览器里输入 http://宿主机IP：8500 访问 web 管理界面。界面上会显示6个绿色的节点。表示所有节点都正常运行中。
## 在 asp.net core 应用内注册服务
好了现在我们已经有了 Consul 集群，现在可以开始编写代码来注册我们的服务了。我们需要完成3点操作。  
1. 定义一个健康检测的接口 
...
```
  ...
  "consul": {
    "clientAddress": "http://192.168.0.117:8700"
  }
```
